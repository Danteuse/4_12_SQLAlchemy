...
def convert_str_to_date(date_str):
	#Конвертирует строку с датой в формате ГГГГ-ММ-ДД в объект datetime.date
...
def main():
	#взаимодействие с пользователем, пользовательский ввод
	session = connect_db()
	user_id = request_data()
	user = session.query(User).filter(User.id == user_id).first()
	if not user:
		print("Такого человека не нашлось:(")
	else:
